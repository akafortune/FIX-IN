...
half3 t2d(float2 p){
   half3 col = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, p ).xyz; //output color _X
   return rgb2yiq( col );
}
...
//main
half3 vhs2( VaryingsDefault i ){

   float2 p = i.texcoord; // normalized tex coordnates 0..1  
   float t = _time;          
   ...
   if(feedbackOn==1){
      half3 fbb = SAMPLE_TEXTURE2D(_FeedbackTex, sampler_FeedbackTex, p ).xyz; //output color
      col = bms(col, fbb*1.);       
   }
